...
function findFirstSingleChar(str) {
  return str.split('').find((item, index, array) => !array.some((it, ind) => it === item && ind !== index)) || null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  interval = isStartIncluded ? `[${interval}` : `(${interval}`;
  interval = isEndIncluded ? `${interval}]` : `${interval})`;
  return interval;
}
...
function isCreditCardNumber(ccn) {
  return ccn
    ...
    .reverse()
    .map((item, index) => {
      if (index % 2 === 1) return ((+item * 2 - 1) % 9) + 1;
      return +item;
    })
...
function getCommonDirectoryPath(pathes) {
  const string = pathes.reduce((acc, item) => {
    let str = '';
    for (let i = 0; i < Math.min(acc.length, item.length); i += 1) {
      if (acc[i] === item[i]) str += acc[i];
      else break;
    }
    return str;
  });
  return string.lastIndexOf('/') === -1 ? '' : string.slice(0, string.lastIndexOf('/') + 1);
}
...
function getMatrixProduct(m1, m2) {
  const matrixRows = m1.length;
  const matrixColumns = m2[0].length;
  const mulCount = m2.length;
  const matrix = [];
  for (let i = 0; i < matrixRows; i += 1) {
    const matrixRow = [];
    for (let j = 0; j < matrixColumns; j += 1) {
      let x = 0;
      for (let k = 0; k < mulCount; k += 1) {
        x += m1[i][k] * m2[k][j];
      }
      matrixRow.push(x);
    }
...
function evaluateTicTacToePosition(position) {
  ...
  const arr = [...position[0], ...position[1], ...position[2]];
